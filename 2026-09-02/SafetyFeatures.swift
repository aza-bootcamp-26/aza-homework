...
var transaction1 = Transaction(id: "TXN-001", date: Date(), amount: 2500.00, description: "Direct Deposit", isDebit: false)
...
var transaction2 = Transaction(id: "TXN-002", date: Date(), amount: 150.00, description: "Grocery Store", isDebit: true)
...
var mockDashboard = AccountDashboard(dataSource: MockAccountDataSource())
mockDashboard.showSummary(for: "ACC-001")
var liveDashboard = AccountDashboard(dataSource: LiveAccountDataSource())
...
class Customer {
    let name: String
    weak var account: Account?    // optional — set after initialization
...
rawBalanceString.flatMap { Double($0) }.map { String(format: "$%.2f", $0) }  
rawInvalidString.flatMap { Double($0) }.map { String(format: "$%.2f", $0) } 
nilString.flatMap { Double($0) }.map { String(format: "$%.2f", $0) }
...
// If a user can input a URL that the program is accessing, a bad actor could input a URL that is malicious
// and somehow does harm to the system. Instead I would use optional binding to make sure that it is safe
...
enum TransferError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .insufficientFunds(let available):
            return "Insufficient funds. Available balance is $\(String(format: "%.2f", available))."
        case .accountNotFound(let id):
            return "Account not found with ID: \(id)."
        case .dailyLimitExceeded(let limit, let attempted):
            return "Daily limit exceeded. Limit is $\(String(format: "%.2f", limit)), attempted transfer is $\(String(format: "%.2f", attempted))."
        case .networkUnavailable:
...
do {
    try executeTransfer(amount: -100, fromBalance: 500, toAccountId: "ACC-002", dailyUsed: 0, dailyLimit: 1000)
} catch {
    print(error.localizedDescription)
}

do {
    try executeTransfer(amount: 100, fromBalance: 500, toAccountId: "", dailyUsed: 0, dailyLimit: 1000)
} catch {
    print(error.localizedDescription)
}

do {
    try executeTransfer(amount: 600, fromBalance: 500, toAccountId: "ACC-002", dailyUsed: 0, dailyLimit: 1000)
} catch {
    print(error.localizedDescription)
}

do {
    try executeTransfer(amount: 600, fromBalance: 500, toAccountId: "ACC-002", dailyUsed: 500, dailyLimit: 1000)
} catch {
    print(error.localizedDescription)
}

do {
    try executeTransfer(amount: 100, fromBalance: 500, toAccountId: "ERR_NET", dailyUsed: 0, dailyLimit: 1000)
} catch {
    print(error.localizedDescription)
}
...
let result1 = try? executeTransfer(amount: 100, fromBalance: 500, toAccountId: "ACC-002", dailyUsed: 0, dailyLimit: 1000)
print(result1 ?? "Transfer failed")

let result2 = try? executeTransfer(amount: -100, fromBalance: 500, toAccountId: "ACC-002", dailyUsed: 0, dailyLimit: 1000)
...
let doubleArray: [Double] = [0.5, 1.5, 2.5]

printFirst(array: intArray)      
printFirst(array: stringArray)   
printFirst(array: doubleArray)   
...
let transactionStack = Stack<Double>()
var mutableStack = transactionStack
mutableStack.push(250.00)
mutableStack.push(45.67)
mutableStack.push(1200.00)
if let popped = mutableStack.pop() {
    print("Popped: \(popped)")
}
if let top = mutableStack.top {
    print("Top: \(top)")        
}
print("Count: \(mutableStack.count)")
...
func findLargest<T: Comparable>(array: [T]) -> T? {
    return array.max()
}
