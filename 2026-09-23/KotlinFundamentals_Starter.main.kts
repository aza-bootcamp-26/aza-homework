...
fun describe(result: TransferResult): String {
    return when (result) {
        is TransferResult.Success -> "Transfer successful! Confirmation ID: ${result.confirmationId}"
...
suspend fun fetchAccounts(): List<Account> {
    ...
    return listOf(
        Account("1", "Alice", 1500.00),
        Account("2", "Bob", -200.50), // Negative balance for testing
        Account("3", "Charlie", 3200.75)
    )
}
...
fun main() = runBlocking {
    ...
    overdraftAccounts.forEach { println(" - ${it.name}: ${it.balance.asCurrency()}") }
}
